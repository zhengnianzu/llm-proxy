...
from utils.triplet_collector import collect_new_triplets, read_session_index_jsonl
...
def erase_src_triplets(
    ...
    from utils.triplet_collector import TIMESTAMP_RE  # noqa: F401 — already imported via collect_new_triplets
...
def run_raw(
    ...
    """
    from utils.triplet_collector import collect_triplets_from_index

    index_path = src / "index.jsonl"
    if not index_path.exists():
        logger.warning("raw mode: src/index.jsonl not found, falling back to scan")
        from utils.triplet_collector import collect_triplets_by_scan
